...
class TCPacketIllustrator:
    ...
    def read_data(self):
        ...
        return [
            ast.literal_eval(line.strip()) for line in lines
    ...
    def populate_sheet(self, sheet, data):
        ...
        for string, size in data.items():
            ...
            while num_cells > 0:
                cells_to_merge = min(num_cells, self.max_cells_per_row - current_col + 1)
                sheet.merge_cells(start_row=current_row, start_column=current_col,
                                  end_row=current_row, end_column=current_col + cells_to_merge - 1)
...
if __name__ == "__main__":
    excel_creator = TCPacketIllustrator('data.txt')
    excel_creator.create_excel('output.xlsx')
